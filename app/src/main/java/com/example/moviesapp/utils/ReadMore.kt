...
open class NoUnderlineClickSpan(val context: Context) : ClickableSpan() {
    override fun updateDrawState(textPaint: TextPaint) {
        ...
        textPaint.color = ContextCompat.getColor(context, R.color.link_blue)
    }
...
fun TextView.setResizableText(
    ...
    if (width <= 0) {
        post {
            setResizableText(fullText, maxLines, viewMore)
        }
        return
    }
    ...
    val suffixText =
        if (viewMore) resources.getString(R.string.see_more) else resources.getString(R.string.see_less)
...
private fun TextView.addClickablePartTextResizable(
    ...
    if (clickableText != null) {
        ...
        val startIndexOffset = if (viewMore) 3 else 0
        builder.setSpan(object : NoUnderlineClickSpan(context) {
            override fun onClick(widget: View) {
                if (viewMore) {
                    setResizableText(fullText, maxLines, false)
                } else {
                    setResizableText(fullText, maxLines, true)
                }
            }
